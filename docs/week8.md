...
- Deepened AWS Auth with IAM Users, Roles, and Policies
- Created own IAM role (Option 1 or 2)
- Read AWS SSO Q&A
- Internalized AWS CLI basics
- Passed quiz
- Left optional MFA enforcement open
Bonus: AWS CLI, IAM, EC2, and S3 Integration
- Project Task 1: AWS CLI with S3
  - List Bucket (Empty):
    - Command: aws s3 ls s3://grocerymate-logs-kb
    - Output: Empty bucket
    - Screenshot: s3_ls_empty_PT1.png
  - Upload File:
    - Command: aws s3 cp test.txt s3://grocerymate-logs-kb/test.txt
    - Output: upload: ./test.txt to s3://grocerymate-logs-kb/test.txt
    - Screenshot: s3_cp_upload_PT1.png
  - Verify Upload:
    - Command: aws s3 ls s3://grocerymate-logs-kb
    - Output: 2025-07-01 13:28:03 19 test.txt
    - Screenshot: s3_ls_test_txt_PT1.png
- Project Task 2: IAM, EC2, and S3
  - EC2 Instance:
    - Instance ID: i-0d64dad897d18f619
    - Name: grocerymate-ec2-tf
    - State: running
    - Screenshot: ec2_instance_PT2.png
  - IAM Role:
    - Name: GrocerymateEC2S3Role
    - Screenshot: iam_role_PT2.png
  - IAM Policy:
    ...
    - Screenshot: iam_policy_PT2.png
  - S3 Bucket:
    - Name: grocerymate-logs-kb
    - Content: test.txt
    - Screenshot: s3_bucket_PT2.png
- Cleanup
...
- Steps:
  ...
  - Select AWS Service: EC2 as the trusted entity. Trust policy:
{
"Version": "2012-10-17",
"Statement": [
{
...
- On Add Permissions, click Create Policy (unavailable in my environment)
- In the JSON tab, add the MFA enforcement policy:
{
"Version": "2012-10-17",
"Statement": [
{
...
- Attach MFAEnforcedS3Policy to the role, remove other policies
- Name the role: MFAEnforcedS3AccessRole
